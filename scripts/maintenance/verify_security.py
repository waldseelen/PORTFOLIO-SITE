...
from apps.main.api_views import validate_notification_data, validate_performance_data
...
def test_contact_form():
    ...
    if form.is_valid():
        ...
        if (
            "<script>" not in clean_data["name"]
            and "<script>" not in clean_data["message"]
        ):
...
def test_file_security():
    ...
    exe_file = SimpleUploadedFile(
        "virus.exe", exe_data, content_type="application/octet-stream"
    )
    ...
    double_ext_file = SimpleUploadedFile(
        "image.jpg.exe", b"fake", content_type="image/jpeg"
    )
...
def test_api_validation():
    ...
    valid_data = {"metric_type": "lcp", "value": 1500.5, "url": "https://example.com"}
    ...
    # Test invalid metric
    invalid_data = {"metric_type": "invalid_metric", "value": 1500}
    ...
    # Test XSS in URL
    xss_data = {"metric_type": "lcp", "value": 1500, "url": 'javascript:alert("xss")'}
