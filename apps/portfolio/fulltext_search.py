...
except Exception as exc:  # psycopg is optional in local/dev
    SearchHeadline = None  # type: ignore
    SearchQuery = None  # type: ignore
    SearchRank = None  # type: ignore
    SearchVector = None  # type: ignore
    TrigramSimilarity = None  # type: ignore
...
class PostgreSQLSearchEngine:
    ...
    def __init__(self):
        ...
        if not POSTGRES_SEARCH_AVAILABLE:
            logger.warning(
                "PostgreSQL full-text search disabled; falling back to basic search (%s)",
    ...
    def full_text_search(
        ...
        if not POSTGRES_SEARCH_AVAILABLE:
            return self._fallback_search(query, models, limit)
    ...
    def _fallback_search(
        self, query: str, models: Optional[List[str]] = None, limit: int = 50
    ) -> Dict[str, Any]:
        """Lightweight search that works without PostgreSQL/psycopg.

        Uses simple icontains filters so local SQLite dev environments can run
        without pulling in PostgreSQL extras.
        """
        results: List[Dict[str, Any]] = []

        # Blog posts
        if not models or "blog" in models:
            blog_qs = Post.objects.filter(title__icontains=query).order_by("-id")[:limit]
            for post in blog_qs:
                results.append(
                    {
                        "id": post.id,
                        ...
                        "url": self._safe_url(post, f"/blog/{getattr(post, 'slug', post.id)}/"),
                        "type": "blog",
                        "category": "Blog Posts",
                        "rank": 0.5,
                        "date": getattr(post, "published_at", getattr(post, "created_at", None)),
                        "tags": list(getattr(post, "tags", [])) if hasattr(post, "tags") else [],
                    }
                )

        # Tools
        if not models or "tools" in models:
            tool_qs = Tool.objects.filter(title__icontains=query).order_by("-id")[:limit]
            for tool in tool_qs:
                results.append(
                    {
                        "id": tool.id,
                        ...
                        "url": self._safe_url(tool, f"/tools/{tool.id}/"),
                        "type": "tool",
                        "category": "Tools",
                        "rank": 0.4,
                        "date": getattr(tool, "created_at", None),
                        "tags": [],
                    }
                )
        ...
        # AI tools
        if not models or "ai_tools" in models:
            ai_qs = AITool.objects.filter(name__icontains=query).order_by("-id")[:limit]
            for ai_tool in ai_qs:
                results.append(
                    {
                        "id": ai_tool.id,
                        "title": getattr(ai_tool, "name", ""),
                        "description": getattr(ai_tool, "description", "")[:240],
                        "url": self._safe_url(ai_tool, getattr(ai_tool, "url", "")),
                        "type": "ai_tool",
                        "category": "AI Tools",
                        "rank": 0.35,
                        "date": getattr(ai_tool, "created_at", None),
                        "tags": [],
                    }
                )
    ...
    def _safe_url(self, obj: Any, default: str = "") -> str:
        """Best-effort URL builder that tolerates missing get_absolute_url."""
        url_callable = getattr(obj, "get_absolute_url", None)
        if callable(url_callable):
            try:
                return url_callable() or default
            except Exception:
                return default
        return default
