...
class SearchAutocompleteView(View):
    ...
    def get(self, request):
        ...
        if not query or len(query) < 2:
            return JsonResponse(
                {"suggestions": [], "query": query, "message": "Query too short"}
            )
        ...
        try:
            ...
            if cached_suggestions:
                return JsonResponse(
                    {"suggestions": cached_suggestions, "query": query, "cached": True}
                )
            ...
            return JsonResponse(
                {"suggestions": suggestions, "query": query, "cached": False}
            )
    ...
    def _get_tool_suggestions(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            tools = Tool.objects.filter(is_visible=True, title__icontains=query).values(
                "title", "id"
            )[:3]
    ...
    def _get_ai_tool_suggestions(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ai_tools = AITool.objects.filter(
                is_visible=True, name__icontains=query
            ).values("name", "url")[:3]
    ...
    def _get_popular_term_suggestions(self, query: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"text": term, "type": "term", "category": "Popular Terms", "score": 0.6}
...
class SearchAPIView(View):
    ...
    def get(self, request):
        ...
        if not query:
            return JsonResponse(
                {"error": "Query parameter is required", "results": [], "total": 0},
                status=400,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Search unavailable", "results": [], "total": 0}, status=500
            )
    ...
    def _log_search_analytics(
        self, query: str, result_count: int, category: str, request
    ):
...
class SearchFiltersView(View):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse({"filters": filters, "cached": True})

        except Exception as e:
            logger.error(f"Search filters API error: {e}")
            return JsonResponse(
                {"error": "Filters unavailable", "filters": {}}, status=500
            )
...
class SearchAnalyticsView(View):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse({"analytics": analytics, "period": f"Last {days} days"})

        except Exception as e:
            logger.error(f"Search analytics API error: {e}")
            return JsonResponse(
                {"error": "Analytics unavailable", "analytics": {}}, status=500
            )
    ...
    def _get_analytics_data(self, days: int) -> Dict[str, Any]:
        ...
        # No result queries
        no_result_queries = [
            search["query"] for search in all_searches if search["result_count"] == 0
        ]
        ...
        top_no_results = sorted(
            no_result_counts.items(), key=lambda x: x[1], reverse=True
        )[:5]
        ...
        return {
            ...
            "average_results": (
                round(sum(result_counts) / len(result_counts)) if result_counts else 0
            ),
            "popular_queries": [
                {"query": query, "count": count} for query, count in popular_queries
            ],
            "no_result_queries": [
                {"query": query, "count": count} for query, count in top_no_results
            ],
            "search_trends": self._calculate_search_trends(all_searches, days),
        }

    def _calculate_search_trends(
        self, searches: List[Dict], days: int
    ) -> Dict[str, int]:
        ...
        for i in range(days):
            date = (timezone.now() - timedelta(days=i)).strftime("%Y-%m-%d")
            daily_count = len([s for s in searches if s["timestamp"].startswith(date)])
...
def popular_searches_api(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Popular searches unavailable", "popular_searches": []},
            status=500,
        )
