'use client';

/**
 * Sanity Studio route - embedded studio experience
 * Access at /studio
 */
export default function StudioPage() {
    return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-100 dark:bg-neutral-900">
            <div className="text-center max-w-lg p-8">
                <h1 className="text-3xl font-bold text-neutral-900 dark:text-white mb-4">
                    Sanity Studio
                </h1>
                <p className="text-neutral-600 dark:text-neutral-400 mb-6">
                    Sanity Studio ayrı bir URL&apos;de barındırılmaktadır.
                    İçerik yönetimi için aşağıdaki bağlantıyı kullanın.
                </p>
                <a
                    href="https://your-project.sanity.studio"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-medium px-6 py-3 rounded-lg transition-colors"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-5 w-5"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                    >
                        <path
                            fillRule="evenodd"
                            d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                            clipRule="evenodd"
                        />
                    </svg>
                    Sanity Studio&apos;ya Git
                </a>
                <p className="text-sm text-neutral-500 dark:text-neutral-500 mt-6">
                    Not: Sanity projenizi oluşturduktan sonra bu URL&apos;yi güncelleyin.
                </p>
            </div>
        </div>
    );
}
